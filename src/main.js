...
// я решил разбить вёрстку спика в разрезе "список/день/событие"
// чтобы, к примеру, при создании пустого списка или нового дня
// не было лишней разметки, которую потом нужно будет удалять/менять/дополнять

// разметка под список дней
const createListTrip = () => {
  return (
    `<ul class="trip-days"></ul>`
  );
};

// разметка отдельного дня
const createTripDay = () => {
  return (
...
// разметка элемента с формой добавления/изменения
